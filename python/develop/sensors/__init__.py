...
VERSION = c_char_p.in_dll(SENSORS_LIB, "libsensors_version").value.decode(
    "ascii"
)
...
def init(config_filename=DEFAULT_CONFIG_FILENAME):
    file_p = stdc.fopen(config_filename, b"r")
    if file_p is None:
    ...
    def __iter__(self):
        ...
        while True:
            result_p = _get_all_subfeatures(
                byref(self.chip), byref(self), byref(number)
            )
            if not result_p:
                break
...
class Bus(Structure):
    ...
    _fields_ = [("type", c_short), ("nr", c_short)]

    def __str__(self):
        return (
            "*"
            if self.type == self.TYPE_ANY
            else _get_adapter_name(byref(self)).decode()
        )

    def __repr__(self):
        return f"{self.__class__.__name__}({self.type!r}, {self.nr!r})"
...
class Chip(Structure):
    ...
    def __new__(cls, *args):
        ...
        if args:
            _parse_chip_name(args[0].encode(), byref(result))
        return result

    def __init__(self, *_args):
        Structure.__init__(self)
        finalize(self, self._destroy)
